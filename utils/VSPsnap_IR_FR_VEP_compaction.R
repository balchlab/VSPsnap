...
#compaction_output<-data.frame(dt=character(),
#                              DCmean=numeric(),
#                              DCsd=numeric(),
#                              VCDmean=numeric(),
#                              VCDsd=numeric(),
...
GetCompactionIndexes<-function(B117,VOC){
dt_vec<-vector()
compaction_output<-vector()                             
#i=228
for(i in c(228,258,289,305,319,336,350,367,381,395,409,426,440,456,470,487,501,517,531,548,562)){     # 228,258,289
  print(i)
  ...
  # avg distance from Centroid + SD (5')
  Alpha_tab<-krige_tab[which(genetab$descriptor%in%B117$descriptor),]
  
  print(dim(Alpha_tab))
  
  mx <- mean(Alpha_tab$seqX[which(Alpha_tab$seqX>0.5)])
  my <- mean(Alpha_tab$IR[which(Alpha_tab$seqX>0.5)])
  
  
  distV<-vector()
  
  for(j in which(Alpha_tab$seqX>0.5)){
    distV[j]<-eucl.dist(Alpha_tab[j,1:2],c(mx,my))
  }
  
  # Variogram cloud only on VOC points
  maxd<-max(dist(Alpha_tab[,1:2])) 
  
  coordinates(Alpha_tab)=~seqX+IR
  
  # generate variogram cloud
  
  vgmcloud <- variogram(FR~1, Alpha_tab, cutoff=maxd, cloud=TRUE)
  
  dt_vec=c(dt_vec,dt)
  
  compaction_output=rbind(compaction_output,c(mx,my,mean(distV,na.rm = T),sd(distV,na.rm = T),mean(vgmcloud$dist),sd(vgmcloud$dist),mean(vgmcloud$gamma),sd(vgmcloud$gamma)))
  
  
  #compaction_output[i,1]<-dt
  #compaction_output[i,2]<-mean(distV,na.rm = T)
  #compaction_output[i,3]<-sd(distV,na.rm = T)
  #compaction_output[i,4]<-mean(vgmcloud$dist)
  #compaction_output[i,5]<-sd(vgmcloud$dist)
  #compaction_output[i,6]<-mean(vgmcloud$gamma)
  #compaction_output[i,7]<-sd(vgmcloud$gamma)
  #compaction_output[i,8]<-mx
  #compaction_output[i,9]<-my
  
}
  
compaction_output=cbind(dt_vec,compaction_output)
colnames(compaction_output)<-c("date","Centroid_x"," Centroid_y","DCmean","DCsd","VCDmean","VCDsd","VCGmean","VCGsd")
  #                              DCsd=numeric(),
  #                              VCDmean=numeric(),
  #                              VCDsd=numeric(),
  #                              VCGmean=numeric(),
...
compaction_output=as.data.frame(compaction_output)
compaction_output[,2:9]=apply(compaction_output[,2:9],2,as.numeric)

compaction_output$date <- factor(compaction_output$date, levels = compaction_output$date)

# plots: https://www.datanovia.com/en/lessons/ggplot-error-bars/

DCmean_plot<-ggplot(compaction_output, aes(date, DCmean)) +
  geom_line() +
  geom_errorbar( aes(ymin = DCmean-DCsd, ymax = DCmean+DCsd),width = 0.2) +
  geom_point(size = 2)
ggsave(DCmean_plot,filename = sprintf("DCmean_%s.png",VOC))

VCDmean_plot<-ggplot(compaction_output, aes(date, VCDmean)) +
  geom_line() +
  geom_errorbar( aes(ymin = VCDmean-VCDsd, ymax = VCDmean+VCDsd),width = 0.2) +
  geom_point(size = 2)
ggsave(VCDmean_plot,filename = sprintf("VCDmean_%s.png",VOC))

VCGmean_plot<-ggplot(compaction_output, aes(date, VCGmean)) +
  geom_line() +
  geom_errorbar( aes(ymin = VCGmean-VCGsd, ymax = VCGmean+VCGsd),width = 0.2) +
  geom_point(size = 2)
ggsave(VCGmean_plot,filename = sprintf("VCGmean_%s.png",VOC))

write.table(compaction_output,quote=F,row.names=F,sep="\t",file =paste0("compaction_output_",VOC,".tsv"))
...
# V2 with 95% CI and confidence band
...
#    }
    ...
    compaction_output=rbind(compaction_output,c(mean(vgmcloud$dist),CI(vgmcloud$dist,ci=0.95)[1],CI(vgmcloud$dist,ci=0.95)[3],mean(vgmcloud$gamma),CI(vgmcloud$gamma,ci=0.95)[1],CI(vgmcloud$gamma,ci=0.95)[3]))
  ...
  compaction_output=cbind(dt_vec,compaction_output)
  colnames(compaction_output)<-c("date","VCDmean","VCDmean_CI_upper","VCDmean_CI_lower","VCGmean","VCGmean_CI_upper","VCGmean_CI_lower")
  #                              DCsd=numeric(),
  #                              VCDmean=numeric(),
  #                              VCDsd=numeric(),
  #                              VCGmean=numeric(),
  ...
  compaction_output=as.data.frame(compaction_output)
  compaction_output[,2:7]=apply(compaction_output[,2:7],2,as.numeric)
  ...
  VCDmean_plot<-ggplot(compaction_output, aes(date, VCDmean)) +
    ...
    geom_point(size = 2) +
    geom_ribbon(aes(ymin = VCDmean_CI_lower, ymax = VCDmean_CI_upper),group=1,alpha=0.2)+
    theme(axis.text.x = element_text(angle = 45, hjust=1,size=14),axis.text.y=element_text(size=14))
    
  ggsave(VCDmean_plot,filename = sprintf("VCDmean_CI_%s.png",VOC))
  
  VCGmean_plot<-ggplot(compaction_output, aes(date, VCGmean)) +
    geom_line(color="blue",group=1) +
    geom_point(size = 2) +
    geom_ribbon(aes(ymin = VCGmean_CI_lower, ymax = VCGmean_CI_upper),group=1,alpha=0.2)+
   theme(axis.text.x = element_text(angle = 45, hjust=1,size=14),axis.text.y=element_text(size=14))
  
  ggsave(VCGmean_plot,filename = sprintf("VCGmean_CI_%s.png",VOC))
  
  write.table(compaction_output,quote=F,row.names=F,sep="\t",file =paste0("compaction_output_",VOC,".tsv"))
  
}

# same but just 7 points following the dates in the actual figure

GetCompactionIndexes<-function(B117,VOC){
  dt_vec<-vector()
  compaction_output<-vector()                             
  #i=228
  for(i in c(105,183,258,336,409,487,562)){      # 228,258,289
    print(i)
  ...
  ggsave(VCDmean_plot,filename = sprintf("VCDmean_7tp_CI_%s.png",VOC))
  ...
  ggsave(VCGmean_plot,filename = sprintf("VCGmean_7tp_CI_%s.png",VOC))
  
  write.table(compaction_output,quote=F,row.names=F,sep="\t",file =paste0("compaction_output_7tp",VOC,".tsv"))
...
# same but just 6 points following the dates in the actual figure
#105
#197
#289
#381
...
GetCompactionIndexes<-function(B117,VOC){
  ...
  compaction_output<-vector()                             
  #i=228
  for(i in c(105,197,289,381,470,562)){      # 228,258,289
    print(i)
  ...
  ggsave(VCDmean_plot,filename = sprintf("VCDmean_6tp_CI_%s.png",VOC))
  ...
  ggsave(VCGmean_plot,filename = sprintf("VCGmean_6tp_CI_%s.png",VOC))
  
  write.table(compaction_output,quote=F,row.names=F,sep="\t",file =paste0("compaction_output_6tp",VOC,".tsv"))
  
}


## Omicron - 6 time points - from 8/15/21 to 1/15/22
# 562 593 623 654 684 715
...
GetCompactionIndexes<-function(B117,VOC){
  ...
  compaction_output<-vector()                             
  #i=228
  for(i in c(562,593,623,654,684,715)){      
    print(i)
...
GetCompactionIndexes(Omicron,"Omicron")
...
vgmcloud <- variogram(FR~1, Alpha_tab, cutoff=maxd*0.75, cloud=TRUE)
vgmcloud_df<-as.data.frame(vgmcloud)

# extract pairwise interactions for VOC considered only
Alpha_rows<-which(genetab$descriptor%in%B117$descriptor)

#vgmcloud_df[which(vgmclo$left%in%c(5,3)&foo$right%in%c(5,3)),]
