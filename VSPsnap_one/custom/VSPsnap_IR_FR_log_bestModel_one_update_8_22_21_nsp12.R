...
nsp12<-read.xlsx("../VSPsnap/data/nsp12.xlsx",sheetName = "Sheet1")
rownames(nsp12)<-nsp12$descriptor
...
for(i in c(105,197,289,381,470,562)){
  print(i)
...
if(contour==T){
    ...
    breaks<-sapply(c(0.1,0.25),function(x) quantile(lzn.kriged_df$var1.var,x))

    pred.plot_contour3=pred.plot+geom_contour(aes(z=var1.var),breaks=breaks,colour="black",size=0.1)
  ...
  dots_tab_nsp12<-dots_tab[which(match(genetab$descriptor,nsp12$descriptor,nomatch=0)>0),]
  dots_tab_nsp12$name<-nsp12[genetab$descriptor[which(match(genetab$descriptor,nsp12$descriptor,nomatch=0)>0)],1] 
  if(nrow(dots_tab_nsp12)>0) dots_tab_nsp12$cl<-"4"
  ...
  # B1617
  if(nrow(dots_tab_nsp12)>0){
  colsL <- c("1" = "brown4", "2" = "green4", "3" = "blue3", "4" = "black")
  pred.plot_contour_nsp12<-pred.plot_contour3+geom_text_repel(data=dots_tab_nsp12,aes(x=seqX,y=IR,label=name,colour=cl),size=4,max.overlaps=Inf)+scale_colour_manual(values=colsL)
  pred.plot_contour_nsp12=pred.plot_contour_nsp12+geom_point(data=dots_tab_nsp12,aes(x=seqX,y=IR,colour=cl),size=1,inherit.aes=F)+guides(colour=FALSE)
  ...
  #pred.plot_contour4_B1617=pred.plot_contour4_B1617+geom_point(data=dots_tab_B1617,aes(x=seqX,y=IR,colour=cl),size=1,inherit.aes=F)+guides(colour=FALSE)
    ...
    pred.plot_contour_nsp12<-pred.plot_contour3
    #pred.plot_contour4_B1617<-pred.plot_contour4
  }
  
  ggsave(pred.plot_contour_nsp12,filename = sprintf("ct_nsp12_%03d.png",i),width = 10, height = 7)
