...
B117<-read.xlsx("../VSPsnap/data/BA1.xlsx",sheetIndex = 1)
rownames(B117)<-B117$descriptor

P1<-read.xlsx("../VSPsnap/data/BA2.xlsx",sheetIndex = 1)
...
if(i<715){
genetab_all<-read.csv(paste0("../VSPsnap/data/cumulative_daily_AF_v4_lag_12_15/",inFiles[i]))
}else{
genetab_all<-read.csv(paste0("../VSPsnap/data/cumulative_daily_AF_v4_lag_01_16_22/",inFiles[i]))}
...
if(contour==T){
  ...
  #dots_tab_Omicron<-dots_tab[which(match(genetab$descriptor,Omicron$descriptor,nomatch=0)>0),]
  #dots_tab_Omicron$name<-Omicron[genetab$descriptor[which(match(genetab$descriptor,Omicron$descriptor,nomatch=0)>0)],10] 
  #if(nrow(dots_tab_Omicron)>0) dots_tab_Omicron$cl<-"5"
  ...
  ggsave(pred.plot_contour3_all,filename = sprintf("ctTen_B1_B2_%03d.png",i),width = 10, height = 7)
  ggsave(pred.plot_contour4_all,filename = sprintf("ctTwoFive_B1_B2_%03d.png",i),width = 10, height = 7)
...
#  ggsave(pred.plot_contour4_B1617,filename = sprintf("ctTwoFive_B1617_%03d.png",i),width = 10, height = 7)
  ...
  # Omicron
#  if(nrow(dots_tab_Omicron)>0){
#    colsL <- c("1" = "black", "2" = "green4", "3" = "blue3", "4" = "brown4","5" = "purple3") # Omicron in black for clarity
#    pred.plot_contour3_Omicron<-pred.plot_contour3+geom_text_repel(data=dots_tab_Omicron,aes(x=seqX,y=IR,label=name,colour=cl),size=4,max.overlaps=Inf)+scale_colour_manual(values=colsL)
#    pred.plot_contour3_Omicron=pred.plot_contour3_Omicron+geom_point(data=dots_tab_Omicron,aes(x=seqX,y=IR,colour=cl),size=1,inherit.aes=F)+guides(colour=FALSE)
    
#    pred.plot_contour4_Omicron<-pred.plot_contour4+geom_text_repel(data=dots_tab_Omicron,aes(x=seqX,y=IR,label=name,colour=cl),size=4,max.overlaps=Inf)+scale_colour_manual(values=colsL)
#    pred.plot_contour4_Omicron=pred.plot_contour4_Omicron+geom_point(data=dots_tab_Omicron,aes(x=seqX,y=IR,colour=cl),size=1,inherit.aes=F)+guides(colour=FALSE)
#  }else{
#    pred.plot_contour3_Omicron<-pred.plot_contour3
#    pred.plot_contour4_Omicron<-pred.plot_contour4
#  }
  
#  ggsave(pred.plot_contour3_Omicron,filename = sprintf("ctTen_Omicron_%03d.png",i),width = 10, height = 7)
#  ggsave(pred.plot_contour4_Omicron,filename = sprintf("ctTwoFive_Omicron_%03d.png",i),width = 10, height = 7)
